...
VimConnection.prototype.showScript = function(debuggerClient, script, line, column) {
  ...
  if (typeof script == 'number') {
    ...
    if (!script) {
      return debuggerClient.reqScripts(function() {
        self.showScript(debuggerClient, scriptRef, line, column);
      });
    }
  }
  ...
  if (debuggerClient.repl) {
    ...
    r.prompt = [script.name, line+1, column].join(':') + '> ';
    r.displayPrompt();
  }
  ...
  getSrcFile(script.name, script.source, function(err, name) {
    ...
    // TODO: extract sourcemap from script source
    // map = convertSourceMap.fromSource(script.source);

    // TODO: find if there is a way to re-use sm generated by coffe at compile time
    // instead of re-compiling (potentially using different compiler)
    if (script.name && script.name.match(/\.coffee$/)) {
    ...
    self.vim.editFile(name, function(buff) {
      ...
      //console.log('adding map for ', script.name, view.sourceMap ? "non-undefined" : "false");
      view.sourceMap = map;
