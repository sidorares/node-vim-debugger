...
VimView.addBreakpointMarkerToBuffer = function(buffer, pos) {
  var line = pos[0];
  var col  = pos[1];
  var anno = buffer.addAnno(VimView.bpAnnoType, line, col);
  var marker = [this.buffer, anno, pos];
  return marker;
}

VimView.prototype.addBreakpointMarker = function(pos) {
  return VimView.addBreakpointMarkerToBuffer(this.buffer, pos);
}
...
VimView.prototype.setCurrentLineMarker = function(pos) {
  ...
  anno = this.buffer.addAnno(VimView.lineAnnoType, line, 0);
